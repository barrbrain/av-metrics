...
pub struct VideoDetails {
    ...
    pub bit_depth: usize,
    /// ChromaSubsampling of the Video.
...
pub fn convert_chroma_data<T: Pixel>(
    ...
    if chroma_pos != ChromaSamplePosition::Vertical {
        ...
        plane_data.copy_from_u8_slice_with_stride(source, source_stride);
        return;
    }

    let get_pixel = if source_bytewidth == 1 {
        fn convert_u8(line: &[u8], index: usize) -> i32 {
            u16::from(line[index]) as i32
        }
        convert_u8
    } else {
        fn convert_u16(line: &[u8], index: usize) -> i32 {
            let index = index * 2;
            (u16::from(line[index + 1]) << 8 | u16::from(line[index])) as i32
        }
        convert_u16
    };
    ...
    for y in 0..height {
        ...
        let out_row = plane_data.row_mut(y).unwrap();
        let breakpoint = cmp::min(width, 2);
        for x in 0..breakpoint {
            out_row[x] = T::try_from(clamp(
